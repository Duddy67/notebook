 ...
 * Note table class
 */
class NotebookTableNote extends JTable
{
  /**
   * Constructor
   *
  ...
  public function bind($array, $ignore = '')
  {
    ...
    if(isset($array['notetext'])) {
      $pattern = '#<hr\s+id=("|\')system-readmore("|\')\s*\/*>#i';
      $tagPos = preg_match($pattern, $array['notetext']);

      if($tagPos == 0) {
	$this->intro_text = $array['notetext'];
	$this->full_text = '';
      }
      else {
  ...
  public function store($updateNulls = false)
  {
    ...
    if($this->id) { // Existing item
      ...
      // New note. A note created and created_by field can be set by the user,
    ...
    $table = JTable::getInstance('Note', 'NotebookTable', array('dbo', $this->getDbo()));

    if($table->load(array('alias' => $this->alias, 'catid' => $this->catid)) && ($table->id != $this->id || $this->id == 0)) {
      $this->setError(JText::_('COM_NOTEBOOK_DATABASE_ERROR_NOTE_UNIQUE_ALIAS'));
      return false;
    }
  ...
  {
    $k = $this->_tbl_key;
    return 'com_notebook.note.'.(int) $this->$k;
  }
  ...
  protected function _getAssetParentId(JTable $table = null, $id = null)
  {
    ...
    // This is a note under a category.
    if($this->catid) {
