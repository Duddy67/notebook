...
class com_notebookInstallerScript
{
  /**
  ...
  function preflight($type, $parent) 
  {
	...
	JFactory::getApplication()->enqueueMessage(JText::_('COM_NOTEBOOK_UPDATE_INCORRECT_VERSION').$rel, 'error');
	return false;
      }
    }
  ...
  function install($parent) 
  {
    ...
    $catData = array('id' => 0, 'parent_id' => 1, 'level' => 1, 'path' => 'notebook',
		     'extension' => 'com_notebook', 'title' => 'Note Book',
		     'alias' => 'notebook', 'description' => '<p>Default category</p>',
  ...
  function uninstall($parent) 
  {
    ...
    $query->delete('#__content_types')
	  ->where('type_alias="com_notebook.note" OR type_alias="com_notebook.category"');
    $db->setQuery($query);
    $db->query();
  }
  ...
  function postflight($type, $parent) 
  {
    if($type == 'install') {
      ...
      $query->where('element='.$db->Quote('com_notebook').' AND type='.$db->Quote('component'));
      $db->setQuery($query);
      $db->query();
      ...
      $query->values($db->Quote('Note Book').','.$db->Quote('com_notebook.note').','.
$db->Quote('{"special":{"dbtable":"#__notebook_note","key":"id","type":"Note","prefix":"NotebookTable","config":"array()"},"common":{"dbtable":"#__ucm_content","key":"ucm_id","type":"Corecontent","prefix":"JTable","config":"array()"}}').','.
...
$db->Quote('NotebookHelperRoute::getNoteRoute'));
      $db->setQuery($query);
      $db->query();
      ...
      $query->insert('#__content_types');
      $query->columns($columns);
      $query->values($db->Quote('Note Book Category').','.$db->Quote('com_notebook.category').','.
...
$db->Quote('NotebookHelperRoute::getCategoryRoute'));
      $db->setQuery($query);
      $db->query();
    }
  }
