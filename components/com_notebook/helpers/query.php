...
/**
 * Note Book Component Query Helper
 *
 * @static
 * @package     Joomla.Site
...
class NotebookHelperQuery
{
  ...
  public static function getQueryDate($orderDate)
  {
    ...
    switch($orderDate) {
      case 'modified' :
	      $queryDate = ' CASE WHEN n.modified = '.$db->quote($db->getNullDate()).' THEN n.created ELSE n.modified END';
	      break;

      // use created if publish_up is not set
      case 'published' :
	      $queryDate = ' CASE WHEN n.publish_up = '.$db->quote($db->getNullDate()).' THEN n.created ELSE n.publish_up END ';
	      break;
  ...
  /**
   * Method to order the intro notes array for ordering
   * down the columns instead of across.
   * The layout always lays the introtext notes out across columns.
   * Array is reordered so that, when notes are displayed in index order
   * across columns in the layout, the result is that the
   * desired note ordering is achieved down the columns.
   *
   * @param   array    &$notes   Array of intro text notes
   ...
   * @since   1.6
   */
  public static function orderDownColumns(&$notes, $numColumns = 1)
  {
    $count = count($notes);

    // Just return the same array if there is nothing to change
    if($numColumns == 1 || !is_array($notes) || $count <= $numColumns) {
      $return = $notes;
    }
    // We need to re-order the intro notes array
    else {
	...
	for($col = 1; $col <= $numColumns; $col++) {
      ...
      // Layout the notes in column order, skipping empty cells
      $i = 0;
	...
	for($row = 1; ($row <= $maxRows) && ($i < $count); $row++) {
      ...
      // Now read the $index back row by row to get notes in right row/col
      ...
      for($row = 1; ($row <= $maxRows) && ($i < $count); $row++) {
	for($col = 1; ($col <= $numColumns) && ($i < $count); $col++) {
	  $return[$keys[$i]] = $notes[$index[$row][$col]];
	  $i++;
	}
      }
    }
