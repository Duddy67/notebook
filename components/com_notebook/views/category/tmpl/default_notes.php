...
<table class="table category table-striped">
  ...
  <tr>
    ...
    <?php if($this->params->get('list_show_author')) : ?>
      <th width="25%">
	<?php echo JText::_('COM_NOTEBOOK_HEADING_CREATE_BY'); ?>
      </th>
    <?php endif; ?>
    <?php if($this->params->get('list_show_date')) : ?>
      <th width="15%" id="categorylist_header_date">
	<?php $date = $this->params->get('order_date'); ?>
	<?php echo JText::_('COM_NOTEBOOK_'.strtoupper($date).'_DATE'); ?>
      </th>
    <?php endif; ?>
  </tr>
  </thead>
  ...
  <tbody>
	...
	<?php  // Build the link to the login page for the user to login or register.
		...
		$returnURL = JRoute::_(NotebookHelperRoute::getNoteRoute($item->slug, $item->catid));
		$link = new JUri($link1);
	...
	<?php if($item->params->get('access-view')) : // Set the link to the note page.
	  ...
	  <?php if($this->params->get('list_show_date')) : ?>
		...
		<?php echo JText::_('COM_NOTEBOOK_UNMODIFIED'); ?>
	      <?php else : ?>
